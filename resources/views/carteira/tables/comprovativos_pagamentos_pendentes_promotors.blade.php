                        ...
                        <div class="col-md-12">
                            ...
                            <div class="card shadow">
                                <div style="background: #3d2822;" class=" card-header text-light">
                                    <h4>Comprovativos de pagamentos dos Promotores pendentes por validar</h4>
                                </div>

                                <div class="card-body">
                                    @if(isset($proof) ? count($proof) < 1 : '')
                                        <div class="d-flex col-md-12 justify-content-center align-items-center alert alert-warning text-center">Nenhum comprovativo de pagamento pendente por validar.</div>
                                        @else
                                        <div class="table-responsive">
                                            ...
                                            <table class="table table-bordered table-hover table-striped">
                                                ...
                                                <tbody>
                                                    @foreach ($proof as $user)
                                                        <tr>
                                                            <td>{{$user->id ?? ''}}</td>
                                                            <td>{{$user->tipo ?? ''}}</td>
                                                            <td>{{$user->status ?? ''}}</td>
                                                            <td>
                                                                @if($user->comprovativo != null)
                                                                <div class="d-flex align-items-center">

                                                                    <a  class="mx-1" target="_blank" href="/storage/files/comprovativo_pagamentos/{{$user->comprovativo ?? ''}}">{{$user->comprovativo ?? ''}}
                                                                    </a>

                                                                    <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="currentColor" class="text-danger bi bi-file-earmark-pdf-fill" viewBox="0 0 16 16">
                                                            ...
                                                            @include('carteira.modals.validar-promotor')
                                                            <a data-toggle="modal" data-target="#validate_artist-{{$user->id}}" class="{{$user->status === 'verificado' ? 'disabled' : '' }} btn-sm  btn btn-primary">Validar</a>
                                                            </div>
                                                            </td>
                                                        </tr>
                                                    @endforeach
